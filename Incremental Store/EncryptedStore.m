...
+ (NSPersistentStoreCoordinator *)makeStoreWithStructOptions:(EncryptedStoreOptions *) options managedObjectModel:(NSManagedObjectModel *)objModel withSqllite:(NSString *) dbName {
  ...
  return [self makeStoreWithOptions:newOptions managedObjectModel:objModel withSqllite:dbName];
}

+ (NSPersistentStoreCoordinator *)makeStore:(NSManagedObjectModel *)objModel passcode:(NSString *)passcode withSqllite:(NSString *) dbName {
  NSDictionary *options = passcode ? @{ EncryptedStorePassphraseKey : passcode } : nil;
  
  return [self makeStoreWithOptions:options managedObjectModel:objModel withSqllite:dbName];
}
...
- (NSIncrementalStoreNode *)newValuesForObjectWithID:(NSManagedObjectID *)objectID
  ...
  if (sqlite3_step(statement) == SQLITE_ROW) {
    ...
    [keys enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
      ...
      id value = [self valueForProperty:property inStatement:statement atIndex:idx + offset];
